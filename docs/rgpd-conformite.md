...
| Incidents (`Incident`) | Aligné sur l'historique d'appels (24‑36 mois) ou la résolution du litige | Suivi qualité/litige rattaché à un BL |
...
- Un compte **non mappé** n'accède à **aucune** donnée (listes vides + message
  `UNMAPPED_MESSAGE`).
- Les **admins** (`ADMIN_EMAILS` dans `lib/permissions.ts`, ou `User.isAdmin` promu
  en base) ont un accès global (`scope.all === true`, helper `requireAdmin()`) —
  réservé à l'administration et aux demandes RGPD.
...
Endpoint d'export implémenté : **`GET /api/rgpd/export?clientId=...`**
(ou `?cardCode=...`), **réservé aux admins** (`requireAdmin`). Il rassemble en JSON,
pour **un seul client**, les données personnelles **déjà stockées** dans TeleVent :
- fiche client (nom, type, commercial, vendeur, téléphones, email, activation),
- emails compta / réception, adresse de facturation,
- contacts (interlocuteurs : nom, fonction, téléphone, email, note),
- historique d'appels CRM (`AppelLog`),
- rappels (`Rappel`),
- incidents (`Incident` : type, note, n° BL, résolution).

L'export **ne touche pas SAP live** : il ne renvoie que des données locales, via
des requêtes **paramétrées** (Prisma typé — aucune injection SQL possible) et en
**lecture seule** (aucune écriture/suppression). Pour une demande couvrant la
source ERP, traiter en parallèle côté SAP B1.
...
L'export RGPD émet a minima un `console.info` (qui a exporté quel client, quand)
côté `app/api/rgpd/export`. **⚠️ TODO** : idéalement une table d'audit dédiée. À
cadrer hors de cet audit (implique une migration DB).
