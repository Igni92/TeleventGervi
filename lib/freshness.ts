/**
 * Fraîcheur / DDM — logique PURE (aucun I/O, aucun import Prisma).
 *
...
/** Nombre de jours « pleins » (heure de Paris) entre aujourd'hui et la DDM. */
export function daysUntilDdm(expirationDate: Date): number {
...
/**
 * Étiquette + ton d'une DDM pour l'affichage :
 *   - `null`/absent → « DDM non saisie » (muted)
 *   - > 3 j         → vert
 *   - 1 à 3 j       → ambre
 *   - ≤ 0 j         → rouge (périmée ou expire aujourd'hui)
 * Le libellé indique l'échéance relative : « DDM J-3 » (dans 3 j), « DDM J+0 »
 * (aujourd'hui), « DDM J+2 » (périmée depuis 2 j).
 */
export function freshnessLabel(
  expirationDate: Date | null | undefined,
): { label: string; tone: FreshnessTone } {
  if (!expirationDate) return { label: "DDM non saisie", tone: "muted" };
...
/** Seuil « DDM proche » : à partir de combien de jours restants on alerte.
 *  Aligné sur le passage au ton ambre de `freshnessLabel` (≤ 3 j). */
export const DDM_SOON_DAYS = 3;

/** Vrai si la DDM mérite une alerte à la vente (proche OU dépassée). */
export function isDdmSoon(expirationDate: Date | string | null | undefined): boolean {
  if (!expirationDate) return false;
  const d = expirationDate instanceof Date ? expirationDate : new Date(expirationDate);
  if (Number.isNaN(d.getTime())) return false;
  return daysUntilDdm(d) <= DDM_SOON_DAYS;
}
