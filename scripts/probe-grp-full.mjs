...
for(const code of [113,115,117]){
  const g=await req("GET",`BusinessPartnerGroups(${code})`,{cookies});
  console.log(`\n=== BPGroup ${code} (${g.body?.Name}) — champs U_* non-null ===`);
  const us=Object.entries(g.body||{}).filter(([k,v])=>k.startsWith("U_")&&v!==null&&v!=="");
  console.log(us.map(([k,v])=>`${k}=${v}`).join(", ") || "(tous null)");
}
