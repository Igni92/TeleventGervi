 ...
 * (Client.commercial = slpName OU Client.vendeur = slpName). Hors scope → 403.
 *
 * Tolère : colonnes credit absentes (migration non appliquée) ou BP absent du
 * miroir → { available: false } (l'encart ne s'affiche pas, pas de bruit).
 */
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Scope slpName : le commercial ne voit que ses clients (commercial OU vendeur).
  ...
  // Lecture du BP miroir (raw SQL — tolérance si la migration n'est pas appliquée).
  try {
    ...
    return NextResponse.json({ available: true, creditLimit, balance, usagePct, overLimit, frozen });
  } catch {
