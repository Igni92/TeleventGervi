    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col divide-y divide-border/50">
        {(rows ?? []).map((c) => {
          ...
          return (
            <div key={c.route} className="flex flex-col gap-1.5 py-3 first:pt-1 last:pb-1 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
              <div className="min-w-0">
                ...
                <p className="text-[11.5px] text-muted-foreground mt-0.5">
                  {c.cadence} · <span className="font-mono">{c.route}</span>
                  {c.detail && <> — {c.detail}</>}
                </p>
              </div>
