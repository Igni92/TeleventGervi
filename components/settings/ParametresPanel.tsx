...
import {
  Moon, Sun, Check, Database, Tags, ChevronRight, CalendarClock,
  Palette, Glasses, MonitorCog, Wand2, CloudSun, FileDown, Activity,
  TimerReset, KeyRound, ShieldAlert,
} from "lucide-react";
...
import {
  ...
  CELEBRATION_MARGIN_DEFAULT, CELEBRATION_EVENT,
  applyAccentPos, ACCENT_POS_DEFAULT, ACCENT_POSITIONS, type AccentPos,
  METEO_ZONE_DEFAULT,
} from "@/components/settings/app-settings";

/**
 * Panneau « Paramètres » — deux ESPACES nettement séparés (SegmentedControl) :
 *
 *   • Préférences (poste)  — Apparence · Confort de lecture · Console · Export.
 *                            Réglages propres au poste, appliqués tout de suite.
 *   • Administration        — Usage & audit · Données SAP · garde-fous · DDM ·
 *                            cron · intégrations (réservé admin/direction).
 *
 * Chaque réglage porte sa description EN CLAIR sous le libellé (motif Réglages
 * iOS) — plus d'info cachée derrière un « ? ». Les cartes de réglage sont
 * SOBRES (fond carte neutre, aucun accent coloré) : ce sont des zones de
 * travail, pas des tuiles de prise d'info.
 *
...
/* ── Brique : groupe de boutons segmentés (réglage à choix multiples) ── */
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            title={o.hint}
            className={cn(
              "inline-flex items-center gap-1.5 px-3 h-8 text-caption font-semibold tracking-tight rounded-md transition-colors",
              // Accent unique (or) sur l'option retenue — aucun halo festif.
              active
...
/**
 * Ligne « libellé + description + contrôle » (motif Réglages iOS) : le libellé
 * en clair, sa description en sous-titre gris VISIBLE, le contrôle à droite.
 */
function SettingRow({
  title, desc, children,
}: {
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-6 py-3.5 first:pt-0 last:pb-0">
      <div className="min-w-0 sm:max-w-md">
        <p className="text-body font-semibold text-foreground">{title}</p>
        {desc && <p className="mt-0.5 text-caption leading-relaxed text-muted-foreground">{desc}</p>}
      </div>
...
function ZoneChip({ label, on, onToggle }: { label: string; on: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      onClick={onToggle}
      className={cn(
        "inline-flex items-center gap-1.5 h-8 px-3 rounded-full text-caption font-semibold transition-colors",
        on
...
/** Position de la barre d'accent colorée des cartes de prise d'info. */
...
/* ── Espaces + sommaire ancré (desktop) avec scroll-spy ───────── */

type Space = "poste" | "admin";

interface SectionDef { id: string; label: string; icon: React.ReactNode; space: Space; adminOnly?: boolean }

const SECTIONS: SectionDef[] = [
  { id: "apparence", label: "Apparence",           icon: <Palette className="h-3.5 w-3.5" />,   space: "poste" },
  { id: "lecture",   label: "Confort de lecture",  icon: <Glasses className="h-3.5 w-3.5" />,   space: "poste" },
  { id: "console",   label: "Console & catalogue", icon: <MonitorCog className="h-3.5 w-3.5" />, space: "poste" },
  { id: "export",    label: "Export & analyse",    icon: <FileDown className="h-3.5 w-3.5" />,  space: "poste" },
  { id: "usage",     label: "Usage & audit",       icon: <Activity className="h-3.5 w-3.5" />,  space: "admin", adminOnly: true },
  { id: "admin",     label: "Administration",      icon: <Database className="h-3.5 w-3.5" />,  space: "admin", adminOnly: true },
];

function SectionNav({ sections, note }: { sections: SectionDef[]; note: string }) {
  ...
  return (
    <nav aria-label="Sections des paramètres" className="hidden lg:block sticky top-6 self-start w-48 shrink-0">
        ...
        {sections.map((s) => (
          <li key={s.id}>
            <a
              href={`#${s.id}`}
              className={cn(
                "flex items-center gap-2 rounded-lg px-2.5 py-2 text-caption font-semibold transition-colors",
                active === s.id
      ...
      <p className="mt-4 px-2.5 text-caption2 leading-relaxed text-muted-foreground/70">{note}</p>
    </nav>
  );
}
...
export function ParametresPanel({ admin = false, userKey = null }: { admin?: boolean; userKey?: string | null }) {
  ...
  const [space, setSpace] = useState<Space>("poste");
  ...
  // Effets festifs au clic : réduits à un unique interrupteur (on/off).
  const [clickFx, setClickFx] = useState<"on" | "off">("on");
  ...
  useEffect(() => {
    ...
    // Effet au clic : on/off (toute valeur d'effet ≠ "off" compte comme activée).
    setClickFx(readSetting(SETTING_KEYS.clickSparks, "sparks") === "off" ? "off" : "on");
    ...
    return onSettingChange((key, value) => {
      ...
      if (key === SETTING_KEYS.clickSparks) setClickFx(value === "off" ? "off" : "on");
  ...
  const spaceSections = SECTIONS.filter((s) => s.space === space && (admin || !s.adminOnly));
  const navNote = space === "admin"
    ? "Réglages globaux (serveur) et outils de synchronisation SAP, réservés à la direction."
    : "Réglages propres à ce poste, appliqués immédiatement sur tous les onglets.";

  return (
    <div className="space-y-6">
      {/* Sélecteur d'espace — visible uniquement quand l'Administration existe. */}
      {admin && (
        <SegmentedControl<Space>
          aria-label="Espace des paramètres"
          value={space}
          onChange={setSpace}
          options={[
            { value: "poste", label: "Préférences" },
      ...
      <div className="flex gap-8 items-start">
        <SectionNav key={space} sections={spaceSections} note={navNote} />
          ...
          {space === "poste" ? (
            <>
              ...
              <section id="apparence" className="scroll-mt-6">
                <SurfaceCard title="Apparence" icon={<Palette className="h-3.5 w-3.5" />}>
                  <div className="divide-y divide-border/50">
                    ...
                    <SettingRow
                      title="Effet festif au clic"
                      desc="Petite animation au clic sur une zone vide (PC uniquement). Coupée d'office quand les animations sont désactivées."
                    >
                      <SegmentToggle
                        ariaLabel="Effet festif au clic"
                        value={clickFx}
                        onChange={(v) => { setClickFx(v); writeSetting(SETTING_KEYS.clickSparks, v === "on" ? "sparks" : "off"); }}
                        options={ONOFF}
                      />
                    </SettingRow>
                    <SettingRow
                      title="Barre d'accent des cartes"
                      desc="Position du liseré coloré sur les cartes de prise d'info (accueil, tableaux de bord)."
                    >
                      <SegmentToggle
              ...
              <section id="lecture" className="scroll-mt-6">
                <SurfaceCard title="Confort de lecture" icon={<Glasses className="h-3.5 w-3.5" />}>
                  <div className="divide-y divide-border/50">
                    <SettingRow
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <span className="tnum text-body font-semibold text-foreground w-12 text-right">
                          {contrast}%
                        </span>
                        {contrastSet && (
                          <button
                            ...
                            className="text-caption font-medium text-muted-foreground hover:text-foreground hover:underline"
                          >
                            Réinit.
                          </button>
                        )}
                    ...
                    <div className="pt-3">
                      <div className="rounded-lg border border-border/60 overflow-hidden">
                        {["Survolez-moi pour tester le réglage", "Ligne d'exemple"].map((t, i) => (
                          <div
                            key={i}
                            className="px-3 py-2 text-caption text-foreground/80 hover:bg-secondary transition-colors cursor-default border-b border-border/40 last:border-0"
                          >
                            {t}
                          </div>
                        ))}
              ...
              <section id="console" className="scroll-mt-6">
                <SurfaceCard title="Console & catalogue" icon={<MonitorCog className="h-3.5 w-3.5" />}>
                  <div className="divide-y divide-border/50">
                    <SettingRow
                      ...
                      >
                        <div className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-secondary/60 px-2.5 ring-1 ring-inset ring-border focus-within:ring-brand-500/60">
                          ...
                          <input
                            ...
                            className="w-56 bg-transparent text-body font-semibold text-foreground outline-none placeholder:font-normal placeholder:text-muted-foreground"
                          />
                        </div>
                      </SettingRow>
                    )}
                    ...
                    {celebOn === "on" && (
                      <SettingRow
                        ...
                        desc="Montant de marge nette (en €) à partir duquel la célébration se déclenche."
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          <div className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-secondary/60 px-2.5 ring-1 ring-inset ring-border focus-within:ring-brand-500/60">
                            <input
                              ...
                              className="w-20 bg-transparent text-right text-body font-semibold tabular-nums text-foreground outline-none"
                            />
                    ...
                    <div className="pt-3">
                      ...
                      >
                        <div className="min-w-0 flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-body font-semibold text-foreground">Gérer les logos de marque</p>
                            <p className="text-caption text-muted-foreground">Associer ou remplacer un logo par marque.</p>
                          </div>
                        </div>
                        <span className="shrink-0 inline-flex items-center gap-1 text-caption font-semibold text-brand-600 dark:text-brand-400">
              ...
              <section id="export" className="scroll-mt-6">
                <SurfaceCard title="Export & analyse" icon={<FileDown className="h-3.5 w-3.5" />}>
                  <StatsExportPanel />
                </SurfaceCard>
              </section>
            </>
            ...
            <>
              ...
              <section id="admin" className="scroll-mt-6 space-y-5">
                <SurfaceCard title="Données SAP" icon={<Database className="h-3.5 w-3.5" />}>
                  <p className="text-caption text-muted-foreground -mt-1 mb-1 max-w-xl">
                  ...
                  <div className="flex flex-col divide-y divide-border/50">
                    <div className="flex flex-col gap-2 py-3 first:pt-1 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                      <div className="min-w-0">
                        <p className="text-body font-semibold text-foreground">Clients SAP</p>
                        <p className="text-caption text-muted-foreground mt-0.5 max-w-md">
                    ...
                    <div className="flex flex-col gap-3 py-3">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-6">
                        <div className="min-w-0">
                          <p className="text-body font-semibold text-foreground">Données stats (miroir comptable)</p>
                    ...
                    <div className="flex flex-col gap-2 py-3 last:pb-1 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
                      <div className="min-w-0">
                        <p className="text-body font-semibold text-foreground">Stock &amp; catalogue produits</p>
                        <p className="text-caption text-muted-foreground mt-0.5 max-w-md">
                ...
                <SurfaceCard title="Garde-fous de vente" icon={<ShieldAlert className="h-3.5 w-3.5" />}>
                  <SafeguardsPanel />
                </SurfaceCard>

                <SurfaceCard title="Fraîcheur · DDM par défaut" icon={<CalendarClock className="h-3.5 w-3.5" />}>
                  <ShelfLifePanel />
                </SurfaceCard>

                <SurfaceCard title="Tâches planifiées (cron)" icon={<TimerReset className="h-3.5 w-3.5" />}>
                  <CronStatusPanel />
                </SurfaceCard>

                <SurfaceCard title="Intégrations · Relances & SAP" icon={<KeyRound className="h-3.5 w-3.5" />}>
