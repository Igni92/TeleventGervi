...
/**
 * Choroplèthe des départements français (métropole + Corse) — remplissage par
 * intensité de la métrique active. Les DOM ne sont pas ici (rendus en bulles
 * sur la carte Outre-mer & Export). Fond : public/geo/fr-departements.json.
 */
export function FranceChoropleth({ zones, metric }: { zones: GeoZone[]; metric: GeoMetric }) {
  ...
  // Zones FR métropolitaines (choroplèthe) indexées par code département.
  ...
  const maxValue = useMemo(() => {
    let max = 0;
    for (const z of zones) if (z.kind === "fr-dept" && z.lat == null) max = Math.max(max, geoValue(z, metric));
    return max;
  }, [zones, metric]);
        ...
        {({ width, height }) => {
          ...
          return (
            <svg width={width} height={height} role="img" aria-label="Carte de France — distribution par département">
              <Mercator
                data={features as unknown as MercatorData}
