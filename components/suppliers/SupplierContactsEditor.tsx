...
/** Interlocuteurs d'un fournisseur — miroir de ContactsEditor (côté client).
 *  Les rôles/postes proviennent d'une liste réutilisable (kind=supplier-contact). */
export function SupplierContactsEditor({ supplierId }: { supplierId: string }) {
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <InfoTip label="Interlocuteurs" content={<>Plusieurs contacts par fournisseur (commercial, qualité, comptabilité…).<br/>Le <b>poste</b> se choisit dans une liste réutilisable que tu peux enrichir.</>} side="right" iconSize={11} />
          </div>
          <p className="text-[12px] leading-snug text-muted-foreground">Contacts rattachés au fournisseur</p>
        </div>
      </div>
          ...
          {contacts.map((c, i) => (
            <li key={c.id} className="rounded-lg border border-border bg-card p-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <TypeCombobox kind="supplier-contact" value={c.role} onChange={(role) => patch(c.id, { role })} placeholder="Poste" className="w-44" />
      ...
      <div className="rounded-lg border border-dashed border-border p-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <TypeCombobox kind="supplier-contact" value={newC.role || null} onChange={(role) => setNewC((c) => ({ ...c, role }))} placeholder="Poste" className="w-44" />
        </div>
        <div className="flex items-center gap-2">
