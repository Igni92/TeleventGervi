...
export function MobileTopBar({ className }: { className?: string }) {
  ...
  return (
    <div
      className={`sticky top-0 z-40 -mx-4 sm:-mx-10 mb-4 overflow-hidden border-b border-border/70 bg-background/80 backdrop-blur-xl ${className ?? ""}`}
    >
        ...
        {session?.user && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                aria-label="Compte"
                className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-brand-500 to-purple-600 text-[12.5px] font-bold text-white shrink-0 shadow-sm active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
              >
                {initials}
              </button>
            </DropdownMenuTrigger>
